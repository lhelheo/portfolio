...
const Technologies = ({ images }: { images: Array<{ src: string, alt: string, description: string }> }) => {
  ...
  return (
    <section className="py-12 bg-[#0f0f0f] flex flex-col items-center">
      ...
      <div className="max-w-[400px] text-center lg:text-left">
        <h4 className="text-xl font-semibold text-[#f2f2f2] mb-4">{images[currentIndex].alt}</h4>
      ...
      <div className="flex justify-center mt-4 space-x-2">
        {images.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full ${index === currentIndex ? "bg-[#f2f2f2]" : "bg-[#555]"}`}
          ></button>
        ))}
      </div>
    </section>
